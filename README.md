...
In order to execute the workflow with default values just run `python .\main.py`. All results are plotted and saved into `./output/plots/XXX` where 'XXX' is either `baseline`, `masking`, `with_explanations` or `comparison_baseline_explanation`. 
...
## Default Fine-Tuning:
- `fine_tune_loop()` (line 34): The `batch_size`, `epochs` and `learning_rate` can be adjusted. 
...
Here, a list of the attribution values is generated that can be investigated by humans to identify tokens that are considered to not be biased.
 - `get_most_meaningful_words()` (line 62): Filter the `top_n` most words and set a `threshold` on how often the word should at least occur.
 - `bullshit_words` (line 68): Fill the list with the words you consider as to be biased.

## Fine-Tuning with Explanation-Based Loss:
Similarly, here we used `n_steps=500` to get representative attributions. 

- `fine_tune_with_explanations()` (line 78): `n_steps`, `batch_size`, `epochs`, `checkpoint_every_n_steps` can be tweaked. Additionally the lambda value `lam` can be tweaked to scale the explanation-based loss.
